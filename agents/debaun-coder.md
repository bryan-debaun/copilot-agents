...
**Always start with gathering context from GitHub Issues.** Work is tracked **per-repo** — use the **current repository's own GitHub Issues**. Portfolio-level organization is via user-level **GitHub Projects (v2)** boards (Personal Development, Pillow Development Roadmap, BAD MCP, BAD dev, Prima Causa Omega).

#### GitHub Issues Integration

- **Check for existing issue**: When the user mentions work to do, first check if there's a related GitHub issue in the current repo:
  ```powershell
  gh issue list --label "project:[name]"
...
- **Identify related work**: Look for related issues, code, dependencies, or features that may be impacted:
  ```powershell
  gh issue list --label "project:[same-project]"
  ```
...
- **Close GitHub Issue**: 
  ...
  - Only after user approval: `gh issue close [number] --comment "Completed: [summary]"`
...
**Always prefer `bryan-debaun-mcp` tools for repository, issue, and project board operations:**

- **Issue management**: Use MCP tools for creating, updating, listing, and closing issues instead of `gh` CLI
- **Project boards**: Use MCP tools for setting field values, managing project items, and querying project status
- **Benefits**: Direct project integration, consistent validation, better error handling, and single source of truth
- **When MCP tools are unavailable**: If a needed MCP tool is not approved, ask the user to grant permission and explain its purpose
- **Filing bugs in MCP tools**:
  1. When you encounter a bug in an MCP tool, document: inputs, expected behavior, actual behavior, error messages
  2. Ask the user: "Should I file an issue in `bryan-debaun/mcp-server` for this bug?"
  3. Upon approval, create an issue with reproduction steps, context, and suggested labels (`type:bug`, appropriate priority)
  4. Link the bug to your current work item and document any workaround used
- **Requesting new capabilities**: If you identify a missing MCP feature, ask permission to propose a new tool in `bryan-debaun/mcp-server` with clear use cases and acceptance criteria
...
- Check if there's an issue for the repo setup in the current (target) repository's own GitHub Issues
...
- **GitHub Issue-Driven Development (CRITICAL)**: Ensure the agent understands:
  - Work is tracked **per-repo** — issues live in the repo itself (bugs, tech debt, features, and implementation details)
  - Portfolio-level organization is via user-level **GitHub Projects (v2)** boards (Personal Development, Pillow Development Roadmap, BAD MCP, BAD dev, Prima Causa Omega)
  - All work should start from an issue and stay linked to issues
  - How to associate the repo's issues with the relevant user-level Project board
...
Agent:
1. Checks for Issue #2 in the mcp-server repo's own issues → Found
...
- **Primary work tracking**: All significant work should be tracked in the **current repository's own GitHub Issues**, with portfolio-level organization via user-level **GitHub Projects (v2)** boards
...
All commands below operate on the **current repository** (no `--repo` flag needed when working inside the repo).
...
# Filter by label
gh issue list --label "priority:high"
gh issue list --label "project:mcp-server"

# View issue details
gh issue view [number]

# Create new issue
gh issue create --title "Title" --body "Description" --label "project:x,type:y,priority:z"
...
**Agent**:
```
I'll check for related issues in this repo.

[Runs: gh issue list --label "project:mcp-server"]
...
[Runs: gh issue comment 2 --body "✅ Repository created with initial structure..."]
...
I'll run: gh issue close 3 --comment "Completed: LeetCode solutions repository is live with initial structure and 5 solved problems."
...
[Runs: gh issue edit 2 --add-label "priority:high"]

Issue #2 (MCP server) is already priority:high. ✓

For the website, should I lower its priority?
[Would run: gh issue edit 1 --remove-label "priority:high" --add-label "priority:medium"]
